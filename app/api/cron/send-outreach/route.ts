...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[Outreach Cron] Time check: ${reason}`);

    // If query string has force=true, we can override time restriction
    const force = request.nextUrl.searchParams.get("force") === "true";

    if (!isHours && !force) {
      return NextResponse.json({
        success: true,
        message: `Skipped: Outside North Star business hours (EST/EDT). ${reason}`
      });
    }

    // Load prospects from Google Sheets
    ...
    if (pendingProspects.length === 0) {
      return NextResponse.json({
        success: true,
        message: "No pending outreach prospects in Google Sheets."
      });
    }

    // Default to sending 1 email per cron trigger to ensure smooth, organic, non-spammy sending pace.
    const limit = Number(request.nextUrl.searchParams.get("limit") || "1");
    const batch = pendingProspects.slice(0, limit);

    console.log(`[Outreach Cron] Processing batch of ${batch.length} emails (limit: ${limit})...`);
    let sentCount = 0;

    for (const prospect of batch) {
      const { subject, html } = compileEmail(prospect);

      console.log(`[Outreach Cron] Sending email via Resend to: ${prospect.email}`);
      ...
      if (result.success) {
        ...
        console.error(`[Outreach Cron] Failed to send email to ${prospect.email}:`, result.error);
        await updateOutreachProspectInSheet(prospect.rowIndex, {
          status: "failed"
        });
      }

      // Safe pause between sends
