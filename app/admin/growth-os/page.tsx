...
export default async function GrowthOSDashboardPage({ searchParams }: { searchParams: Promise<{ key?: string }> }) {
  ...
  const pathToTarget = report?.pathToTarget;
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-100"><Header /><main className="mx-auto max-w-7xl px-4 py-10">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-2">
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6"><h2 className="text-lg font-black">CEO decision</h2><div className="mt-4 rounded-xl border border-amber-900 bg-amber-950/40 p-4"><div className="text-xs font-bold uppercase tracking-wider text-amber-400">Primary bottleneck</div><p className="mt-2 text-sm">{report?.decisionBasis?.primary_bottleneck || 'Unavailable'}</p></div><div className="mt-4 rounded-xl bg-slate-950 p-4"><div className="text-xs font-bold uppercase tracking-wider text-slate-500">Current directive</div><p className="mt-2 text-sm text-slate-300">{report?.decisionBasis?.decision || 'Unavailable'}</p></div><p className="mt-4 text-xs text-slate-500">The 30-day operating target remains $10K verified cash. The first checkpoint is $1K, not a reduced target. Strict $10K MRR still requires 345 active $29 memberships. Call-dependent $199 products remain outside automated distribution.</p></section>
    </div>

    <section className="mt-6 rounded-2xl border border-indigo-800 bg-indigo-950/30 p-6"><h2 className="text-lg font-black">$10K distribution capacity required</h2><p className="mt-1 text-xs text-slate-400">Actual 30-day evidence is shown against the remaining operating requirement. Targets are not claimed results.</p><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl bg-slate-950 p-4"><div className="text-xs font-bold uppercase text-slate-500">Paid orders</div><div className="mt-1 text-2xl font-black">{pipeline.completedPurchasesCount || 0} <span className="text-sm text-slate-500">/ {pathToTarget?.requiredOrders || 0}</span></div></div><div className="rounded-xl bg-slate-950 p-4"><div className="text-xs font-bold uppercase text-slate-500">Checkout starts</div><div className="mt-1 text-2xl font-black">{pipeline.checkoutStartsCount || 0} <span className="text-sm text-slate-500">/ {pathToTarget?.requiredCheckouts || 0}</span></div></div><div className="rounded-xl bg-slate-950 p-4"><div className="text-xs font-bold uppercase text-slate-500">Product visitors</div><div className="mt-1 text-2xl font-black">{pipeline.productVisitors30d || 0} <span className="text-sm text-slate-500">/ {pathToTarget?.requiredProductVisitors || 0}</span></div></div><div className="rounded-xl bg-slate-950 p-4"><div className="text-xs font-bold uppercase text-slate-500">Human sessions</div><div className="mt-1 text-2xl font-black">{pipeline.uniqueSessions30d || 0} <span className="text-sm text-slate-500">/ {pathToTarget?.requiredRawTraffic || 0}</span></div></div></div><div className="mt-4 rounded-xl border border-indigo-900 bg-slate-950/60 p-4 text-xs text-indigo-100">Paid recommendation impressions: <strong>{pipeline.paidOfferImpressions30d || 0}</strong>. Target funnel: 50% session → product decision · 6% product visitor → checkout · 40% checkout → provider capture. Growth OS reports actual performance at each stage.</div></section>
