...
export default async function GrowthOSDashboardPage({ searchParams }: { searchParams: Promise<{ key?: string }> }) {
  ...
  const cards = [
    ...
    { label: 'Verified MTD revenue', value: money(scoreboard?.currentVerifiedRevenueUSD), sub: `Target ${money(scoreboard?.monthlyRevenueTargetUSD)}`, icon: Activity },
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-100"><Header /><main className="mx-auto max-w-7xl px-4 py-10">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-2">
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6"><h2 className="text-lg font-black">CEO decision</h2><div className="mt-4 rounded-xl border border-amber-900 bg-amber-950/40 p-4"><div className="text-xs font-bold uppercase tracking-wider text-amber-400">Primary bottleneck</div><p className="mt-2 text-sm">{report?.decisionBasis?.primary_bottleneck || 'Unavailable'}</p></div><div className="mt-4 rounded-xl bg-slate-950 p-4"><div className="text-xs font-bold uppercase tracking-wider text-slate-500">Current directive</div><p className="mt-2 text-sm text-slate-300">{report?.decisionBasis?.decision || 'Unavailable'}</p></div><p className="mt-4 text-xs text-slate-500">Strict $10K MRR requires 345 active $29 memberships. One-time $19/$49/$79/$199 products count toward monthly revenue, not MRR.</p></section>
    </div>
