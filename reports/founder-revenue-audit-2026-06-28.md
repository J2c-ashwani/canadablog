# Founder Revenue & Search Intent Audit

Generated: 7/2/2026 | Source: `fsidigital.ca-Performance-on-Search-2026-06-28`

This audit evaluates FSI Digital's organic pages based on conversion viability and expected business value. Rather than focusing on raw vanity traffic, it programmatically categorizes page search intent and models customer lifetime value (LTV).

## 📊 High-Level Breakdown

*   **Total Scanned Pages**: 309
  *   **Transactional Intent**: 88 pages
  *   **Commercial Investigation**: 59 pages
  *   **Informational Intent**: 162 pages
*   **Tiers**: Tier 1 (26) | Tier 2 (11) | Tier 3 (272)
*   **Upgraded Pages**: 273 | **Pending Pages**: 36

---

## 📈 System-Level Funnel Scenarios (MRR Modeling)
To scale to **$10k+ MRR**, we track conversions at the system funnel level. This model projects Monthly Recurring Revenue (MRR) across three scenarios, showing both raw Report AOV ($120) and blended customer LTV ($275):

| Scenario | Organic Visits / Mo | Calculator CVR | Purchase CVR | Calculator Starts | Est. Monthly Purchases | Est. MRR (Report Only) | Est. MRR (Blended LTV) |
|---|---|---|---|---|---|---|---|
| **Conservative** | 5,000 | 4.0% | 2.0% | 200 | 4.0 | $480 | **$1,100** |
| **Expected** | 15,000 | 5.0% | 3.0% | 750 | 22.5 | $2,700 | **$6,187** |
| **Ambitious** | 50,000 | 6.0% | 4.0% | 3,000 | 120.0 | $14,400 | **$33,000** |

> [!NOTE]
> **Blended LTV Calculation ($275)**:
> * Initial Report purchase: $120
> * 15% conversion to $199 Strategy Session / Audit = +$30 expected value
> * 5% conversion to $2,500 Full Funding Application service = +$125 expected value

---

## 🚀 Founder Revenue Dashboard (Top 25 Revenue Pillars)
This dashboard attributes expected revenue directly to individual search pages based on search intent, GSC baseline positioning, and conversion statistics:

| Rank | Tier | Path | Intent | Est. Monthly Traffic | Calculator Starts | Est. Annual Revenue Impact | Backlink Priority | Status |
|---|---|---|---|---|---|---|:---:|---|
| 1 | `Tier 1` | `/blog/csbfp-canada-small-business-financing-program` | `Transactional` | 8 | 0.4 | **$11** | Low | ✅ Upgraded |
| 2 | `Tier 1` | `/guides/apply-irap-grants` | `Transactional` | 4 | 0.2 | **$6** | ⭐ **High** | ✅ Upgraded |
| 3 | `Tier 1` | `/guides/irap-innovation-application-guide` | `Transactional` | 4 | 0.2 | **$6** | ⭐ **High** | ✅ Upgraded |
| 4 | `Tier 1` | `/guides/women-entrepreneurship-loan-fund-guide` | `Transactional` | 4 | 0.2 | **$6** | Low | ✅ Upgraded |
| 5 | `Tier 1` | `/guides/apply-csbfp-loans` | `Transactional` | 4 | 0.2 | **$6** | Low | ✅ Upgraded |
| 6 | `Tier 1` | `/blog/women-entrepreneurship-strategy-canada` | `Informational` | 23 | 0.1 | **$3** | Low | ✅ Upgraded |
| 7 | `Tier 1` | `/blog/women-entrepreneurship-loan-fund-canada` | `Transactional` | 2 | 0.1 | **$3** | Low | ✅ Upgraded |
| 8 | `Tier 1` | `/guides/apply-women-entrepreneurship-strategy` | `Commercial Investigation` | 4 | 0.1 | **$3** | Low | ✅ Upgraded |
| 9 | `Tier 1` | `/guides/women-entrepreneurship-fund-guide` | `Commercial Investigation` | 4 | 0.1 | **$3** | Low | ✅ Upgraded |
| 10 | `Tier 1` | `/blog/canada-irap-grants-2026` | `Transactional` | 1 | 0.1 | **$2** | ⭐ **High** | ✅ Upgraded |
| 11 | `Tier 1` | `/blog/csbfp-canada-small-business-financing-program-government-grants` | `Transactional` | 1 | 0.0 | **$1** | Low | ✅ Upgraded |
| 12 | `Tier 1` | `/blog/ontario-small-business-grants-guide` | `Transactional` | 1 | 0.0 | **$1** | ⭐ **High** | ✅ Upgraded |
| 13 | `Tier 1` | `/blog/irap-industrial-research-assistance-program-government-grants` | `Transactional` | 1 | 0.0 | **$1** | ⭐ **High** | ✅ Upgraded |
| 14 | `Tier 1` | `/blog/irap-industrial-research-assistance-program` | `Transactional` | 1 | 0.0 | **$1** | ⭐ **High** | ✅ Upgraded |
| 15 | `Tier 1` | `/guides/apply-irap-government-grants` | `Transactional` | 1 | 0.0 | **$1** | ⭐ **High** | ✅ Upgraded |
| 16 | `Tier 1` | `/guides/sred-application-guide` | `Transactional` | 1 | 0.0 | **$1** | ⭐ **High** | ✅ Upgraded |
| 17 | `Tier 1` | `/guides/apply-csbfp-government-financing` | `Transactional` | 1 | 0.0 | **$1** | Low | ✅ Upgraded |
| 18 | `Tier 1` | `/blog/women-entrepreneurship-grants-2026` | `Informational` | 3 | 0.0 | **$0** | Low | ✅ Upgraded |
| 19 | `Tier 1` | `/blog/canada-irap-grants-2025` | `Transactional` | 0 | 0.0 | **$0** | ⭐ **High** | ✅ Upgraded |
| 20 | `Tier 1` | `/blog/sred-tax-credits-2025` | `Transactional` | 0 | 0.0 | **$0** | ⭐ **High** | ❌ Pending |
| 21 | `Tier 1` | `/blog/sred-tax-credits-2026` | `Transactional` | 0 | 0.0 | **$0** | ⭐ **High** | ✅ Upgraded |
| 22 | `Tier 1` | `/blog/sred-tax-credits-vs-cdap-canadian-founders` | `Transactional` | 0 | 0.0 | **$0** | ⭐ **High** | ✅ Upgraded |
| 23 | `Tier 1` | `/blog/irap-industrial-research-assistance-program-innovation` | `Transactional` | 0 | 0.0 | **$0** | ⭐ **High** | ✅ Upgraded |
| 24 | `Tier 1` | `/blog/sred-scientific-research-experimental-development` | `Transactional` | 0 | 0.0 | **$0** | ⭐ **High** | ✅ Upgraded |
| 25 | `Tier 1` | `/blog/women-entrepreneurship-strategy-canada-government-grants` | `Informational` | 2 | 0.0 | **$0** | Low | ✅ Upgraded |
---

## 🎯 Priority Wave 3 Candidates (Actionable Next Steps)
The top 15 **pending** candidate pages sorted by their priority Tier and expected annual revenue. Wave 3 updates should strictly focus on these high-leverage assets:

| Rank | Tier | Path | Raw Title | GSC Impressions | GSC Avg Position | Freshness Score | Est. Annual Revenue Impact | Source File |
|---|---|---|---|---|---|---|---|---|
| 1 | `Tier 1` | `/blog/sred-tax-credits-2025` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/canada-news/sred-tax-credits-2025.ts` |
| 2 | `Tier 2` | `/blog/canada-regional-development-2025` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/canada-news/canada-regional-development-2025.ts` |
| 3 | `Tier 3` | `/blog/q4-2026-deadlines` | Q4 2026 Grant Deadlines: End-of-Year Funding Rush | 15 | 8.7 | 2/10 | **$0** | `lib/data/blog-posts/funding-alerts/q4-2026-deadlines.ts` |
| 4 | `Tier 3` | `/blog/agricultural-innovation-2025` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/canada-news/agricultural-innovation-2025.ts` |
| 5 | `Tier 3` | `/blog/canexport-grants-2025` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/canada-news/canexport-grants-2025.ts` |
| 6 | `Tier 3` | `/blog/clean-technology-2025` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/canada-news/clean-technology-2025.ts` |
| 7 | `Tier 3` | `/blog/digital-transformation-2025` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/canada-news/digital-transformation-2025.ts` |
| 8 | `Tier 3` | `/blog/innovation-superclusters-2025` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/canada-news/innovation-superclusters-2025.ts` |
| 9 | `Tier 3` | `/blog/minority-business-grants-2025` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/demographic-specific/minority-business-grants-2025.ts` |
| 10 | `Tier 3` | `/blog/rural-business-development-2025` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/demographic-specific/rural-business-development-2025.ts` |
| 11 | `Tier 3` | `/blog/women-business-grants-2025` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/demographic-specific/women-business-grants-2025.ts` |
| 12 | `Tier 3` | `/blog/october-2025-last-chance` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/funding-alerts/october-2025-last-chance.ts` |
| 13 | `Tier 3` | `/blog/october-2026-last-chance` | October 2026: Last Chance Grant Deadlines Before Year-End | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/funding-alerts/october-2026-last-chance.ts` |
| 14 | `Tier 3` | `/blog/q4-2025-deadlines` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/funding-alerts/q4-2025-deadlines.ts` |
| 15 | `Tier 3` | `/blog/cybersecurity-grants` | Unknown | 0 | 0.0 | 2/10 | **$0** | `lib/data/blog-posts/industry-specific/cybersecurity-grants.ts` |

---

## 🔗 Backlink Strategy Matrix
These are our "money pages" where backlink campaigns should be focused to yield the highest business conversion return:

| Page Path | Page Title | Intent | GSC Average Position | Est. Annual Revenue Impact |
|---|---|---|---|---|
| `/guides/apply-irap-grants` | How to Apply for IRAP Innovation Grants | `Transactional` | 11.7 | **$6** |
| `/guides/irap-innovation-application-guide` | IRAP Innovation Funding Application Guide | `Transactional` | 11.0 | **$6** |
| `/blog/canada-irap-grants-2026` | Canada Irap Grants 2026 | $400M+ Funding Guide | `Transactional` | 29.9 | **$2** |
| `/blog/ontario-small-business-grants-guide` | Ontario Small Business Grants 2026 | $2.1B+ Available for Ontario SMEs | `Transactional` | 18.1 | **$1** |
| `/blog/irap-industrial-research-assistance-program-government-grants` | IRAP Industrial Research Assistance Program Canada 2026 | Government R&D Funding Guide | `Transactional` | 26.8 | **$1** |
| `/blog/irap-industrial-research-assistance-program` | Industrial Research Assistance Program (IRAP) 2026 | Up to $1M Tech Grants | `Transactional` | 18.8 | **$1** |
| `/guides/apply-irap-government-grants` | IRAP Government Grants Application Process | `Transactional` | 0.0 | **$1** |
| `/guides/sred-application-guide` | SR&ED Tax Credit Application Guide | `Transactional` | 0.0 | **$1** |
| `/blog/canada-irap-grants-2025` | Canada IRAP Grants 2025: Industrial Research Assistance Program - $400M+ Innovation Powerhouse | `Transactional` | 0.0 | **$0** |
| `/blog/sred-tax-credits-2025` | Unknown | `Transactional` | 0.0 | **$0** |
