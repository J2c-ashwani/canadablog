...
for (const [slug, newAnswer] of Object.entries(newAnswers)) {
    ...
    if (content.match(regex)) {
        // Escape the $ sign in the new answer string so it doesn't get treated as a backreference!
        const escapedNewAnswer = newAnswer.replace(/\$/g, '$$$$');
        content = content.replace(regex, `$1"${escapedNewAnswer}"`);
        updatedCount++;
        console.log(`✅ Fixed syntax in short answer for: ${slug}`);
    } else {
