...
async function run() {
  ...
  assert(calculatorComponent.includes("useState<'funding-match-report' | 'funding-bundle' | 'funding-roadmap'>('funding-match-report')"), 'Calculator defaults new buyers to the lowest-friction $19 entry product');
  assert(!calculatorComponent.includes('calc-phone-gate') && calculatorComponent.includes('setConsentToPartnerContact(event.target.checked)'), 'Calculator removes the mandatory phone gate and offers an explicit optional marketing opt-in');
  assert(!calculatorComponent.includes('window.location.href = `/audit?email=') && calculatorComponent.includes("isEnterprise) {\n                setSelectedProductId('funding-bundle')"), 'Enterprise calculator leads stay in the self-serve $79 flow instead of being diverted to a call');
