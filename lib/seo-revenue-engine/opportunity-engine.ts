...
/**
 * FSI Opportunity Engine (War Mode v1.0)
 * 
 * Ingests GSC performance data, filters by commercial intent,
 * calculates Revenue Opportunity Score & Expected Monthly Cash,
 * and extracts the top attackable SEO revenue targets.
 */
...
export class OpportunityEngine {
  public static async getScoredOpportunities(): Promise<SEORevenueOpportunity[]> {
    ...
    for (const item of rawData) {
      ...
      // Commercial Offer & Conversion Modeling
      ...
      // Expected Conversion Rate (from organic visit to paid checkout):
      ...
      // Calculate Revenue Opportunity Score (0 to 100)
      ...
      const commercialIntentScore = classification.tier === 'TIER_A_MONEY' ? 100 : (classification.tier === 'TIER_B_COMMERCIAL_RESEARCH' ? 70 : 40)
      ...
      const competitorDifficulty = position <= 10 ? 'LOW' : (position <= 25 ? 'MEDIUM' : 'HIGH')

      // Generate exact Title/Meta & Content Attack plans
