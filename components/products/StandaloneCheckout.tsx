...
function trackCheckoutStage(input: {
  eventName: string;
  productId: string;
  revenue: number;
...
export function StandaloneCheckout({ productId, price, productName }: StandaloneCheckoutProps) {
  ...
  const emailReadyTrackedRef = useRef(false);
  ...
  useEffect(() => {
    trackCheckoutStage({
      eventName: 'product_checkout_viewed',
      productId,
      revenue: price,
  ...
  useEffect(() => {
    if (!isEmailValid || emailReadyTrackedRef.current) return;
    emailReadyTrackedRef.current = true;
    trackCheckoutStage({
      eventName: 'checkout_delivery_email_ready',
      productId,
      revenue: price,
      metadata: { checkoutVersion: 'focused-v2' },
    });
  }, [isEmailValid, productId, price]);
  ...
  useEffect(() => {
    if ((window as any)[PAYPAL_PRODUCT_NAMESPACE]?.Buttons) {
      setSdkReady(true);
      return;
    }
    ...
    const markReady = () => {
      if ((window as any)[PAYPAL_PRODUCT_NAMESPACE]?.Buttons) {
        setSdkReady(true);
        trackCheckoutStage({
          eventName: 'paypal_sdk_ready',
          productId,
          revenue: price,
          metadata: { checkoutVersion: 'focused-v2', namespace: PAYPAL_PRODUCT_NAMESPACE },
        });
      } else {
        setPaymentError('Could not initialize secure checkout. Please refresh the page.');
      }
    };
    ...
    const markFailed = () => {
      ...
      trackCheckoutStage({
        eventName: 'paypal_sdk_load_failed',
        productId,
        revenue: price,
        metadata: { checkoutVersion: 'focused-v2' },
      });
    ...
    const existing = document.querySelector<HTMLScriptElement>('script[data-fsi-product-paypal="true"]');
    if (existing) {
    ...
    script.dataset.fsiProductPaypal = 'true';
    script.setAttribute('data-namespace', PAYPAL_PRODUCT_NAMESPACE);
  ...
  }, [paypalClientId, productId, price]);
  ...
  useEffect(() => {
    const paypal = (window as any)[PAYPAL_PRODUCT_NAMESPACE];
    if (!sdkReady || !isEmailValid || !paypal) return;
    ...
    try {
      const buttons = paypal.Buttons({
        ...
        createOrder: async () => {
          ...
          trackCheckoutStage({
            eventName: 'paypal_button_clicked',
        ...
        onApprove: async (data: any) => {
          try {
            ...
            trackCheckoutStage({
              eventName: 'paypal_payment_approved',
              productId: finalProductId,
              revenue: finalPrice,
              metadata: { checkoutVersion: 'focused-v2' },
            });
            ...
            trackCheckoutStage({
              eventName: 'paypal_capture_failed',
              productId: finalProductId,
              revenue: finalPrice,
              metadata: { checkoutVersion: 'focused-v2' },
            });
        ...
        onCancel: () => {
          trackCheckoutStage({
            eventName: 'paypal_checkout_cancelled',
            productId: finalProductId,
            revenue: finalPrice,
            metadata: { checkoutVersion: 'focused-v2' },
          });
      ...
      if (!buttons.isEligible()) {
        setPaymentError('PayPal checkout is not available for this browser or account.');
        return;
      }
      const buttonKey = `${finalProductId}:${finalPrice}`;
      void buttons.render('#standalone-paypal-button').then(() => {
        if (renderedButtonKeyRef.current === buttonKey) return;
        renderedButtonKeyRef.current = buttonKey;
        trackCheckoutStage({
          eventName: 'paypal_buttons_rendered',
          productId: finalProductId,
          revenue: finalPrice,
          metadata: { checkoutVersion: 'focused-v2' },
        });
        ...
        setPaymentError('Could not display secure checkout. Please refresh the page.');
        trackCheckoutStage({
          eventName: 'paypal_buttons_render_failed',
          productId: finalProductId,
          revenue: finalPrice,
          metadata: { checkoutVersion: 'focused-v2' },
        });
      ...
      <div className="space-y-5">
          ...
          <div className="bg-slate-900/60 border border-slate-700/50 rounded-xl p-4 space-y-2">
            ...
            <div className="space-y-1.5 pt-1 text-[10.5px] text-slate-300">
              <div className="flex items-center gap-2">
                ...
                <span>
                  {productId === 'funding-toolkit' && 'Six downloadable application templates and financial models'}
                  {productId === 'funding-approval-library' && 'Proposal framing examples and reference material'}
                  {productId === 'funding-roadmap' && 'Prioritized four-month application sequence'}
                  {productId === 'funding-match-report' && 'Personalized program matches and eligibility requirements'}
                  {productId === 'funding-bundle' && 'Match report, action plan, and stacking simulation'}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle className="w-3.5 h-3.5 text-emerald-450 shrink-0" />
                <span>Instant on-screen access with an email recovery link</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="space-y-3.5 pt-1">
          <div>
            ...
            <p className="text-[10.5px] text-slate-400 mt-1 leading-normal">
              Enter the email where you want your purchase and secure recovery link delivered.
            </p>
          </div>
              ...
              >
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-[10px] text-slate-400 leading-normal mt-0.5">
                    Add proposal framing examples and reference budgets. (Normally $9 separately).
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="border-t border-slate-700/40 pt-4 space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Prepared and Quality-Checked</h4>
